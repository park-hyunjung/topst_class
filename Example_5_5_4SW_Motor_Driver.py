...
switch_cw_state_cur = 0
switch_cw_state_pre = 0
switch_cw_event_flag = 0

switch_ccw_state_cur = 0
switch_ccw_state_pre = 0
switch_ccw_event_flag = 0
...
def setup_all_pins():
    ...
    gpio_setup_input(SWITCH_CW_PIN)
    gpio_setup_input(SWITCH_CCW_PIN)
...
def cleanup_all_pins():
    ...
    gpio_cleanup(SWITCH_CW_PIN)
    gpio_cleanup(SWITCH_CCW_PIN)
...
try:
    ...
    # 초기화 메시지 출력
    print("2개의 스위치는 Up/Down / 2개의 스위치는 Duty Rate")
    ...
    while True:
        ...
        switch_cw_state_cur = gpio_read(SWITCH_CW_PIN)
        switch_ccw_state_cur = gpio_read(SWITCH_CCW_PIN)
        ...
        if switch_cw_state_cur == 0:
            if switch_cw_state_pre == 1:  # falling edge detect
                time.sleep(0.02)  # 바운스 방지

                switch_cw_event_flag = 1

        switch_cw_state_pre = switch_cw_state_cur

        if switch_ccw_state_cur == 0:
            if switch_ccw_state_pre == 1:  # falling edge detect
                time.sleep(0.02)  # 바운스 방지

                switch_ccw_event_flag = 1

        switch_ccw_state_pre = switch_ccw_state_cur
        ...
        if switch_up_event_flag == 1:
            ...
            pwm_set_duty_cycle(PWM_NUM, duty_cycle_ms)
            print(f"DC-Motor Duty cycle increased: {duty_cycle_ms}")
        ...
        if switch_down_event_flag == 1:
            ...
            pwm_set_duty_cycle(PWM_NUM, duty_cycle_ms)
            print(f"DC-Motor Duty cycle decreased: {duty_cycle_ms}")

        if switch_cw_event_flag == 1:
            switch_cw_event_flag = 0

            gpio_write(M_IN1, 1)
            gpio_write(M_IN2, 0)
            print(f"DC-Motor Direction : CW")

        if switch_ccw_event_flag == 1:
            switch_ccw_event_flag = 0

            gpio_write(M_IN1, 0)
            gpio_write(M_IN2, 1)
            print(f"DC-Motor Direction : CCW")
